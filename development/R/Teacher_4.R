  ...
  dashboardBody(
    tabsetPanel(
      tabPanel("Анализ группы",
               ...
               fluidRow(
                 box(
                   title = "Статистика активности студентов",
                   status = "primary",
                   solidHeader = TRUE,
                   width = 6,
                   plotOutput("activity_stats_plot", height = "500px"),
                 ),
                 box(
...
server <- function(input, output, session) {
  ...
  # Функция для анализа активности студентов
  analyze_student_activity <- function(data) {
    req(data)
    
    activity_stats <- data %>%
      select(-all_of(final_grade_col())) %>%
      pivot_longer(
        cols = -`Адрес электронной почты`,
        names_to = "Задание",
        values_to = "Оценка"
      ) %>%
      mutate(
      ...
      group_by(`Адрес электронной почты`) %>%
      summarise(
        Всего_заданий = n(),
        Сдано_заданий = sum(Сдано),
        Процент_сдачи = round(Сдано_заданий / Всего_заданий * 100, 1),
        .groups = "drop"
      )
  ...
  output$activity_stats_plot <- renderPlot({
    req(data_clean())
    
    activity_data <- analyze_student_activity(data_clean())
    
    ggplot(activity_data, aes(x = reorder(`Адрес электронной почты`, Процент_сдачи), 
                              y = Процент_сдачи)) +
      geom_col(fill = "#4E79A7") +
      geom_text(aes(label = paste0(Процент_сдачи, "%")), 
                hjust = -0.1, size = 3) +
      coord_flip() +
      labs(x = "Студент", y = "Процент выполненных заданий") +
      theme_minimal() +
